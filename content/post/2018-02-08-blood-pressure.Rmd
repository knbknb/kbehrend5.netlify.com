...
```{r fig.cap="Blood Pressure by Gender, NHANES Dataset."}
bp2 <- bloodpressure2 %>% 
...
htmltools::HTML(sprintf('<div class="alert alert-warning">So the rate is more like <b>%s mm Hg/year.</b> Half as much as for the toy dataset.</div>', bpr))
...
The significance of Age and Body Weight is very high, but the general correlation is very weak ( R^2 = `r  summary(bloodpressure2_model)$r.squared`). This can also be seen in the following plots. 

```{r fig.cap="Predicted vs Actual Blood Pressure."}
...
Comparing this with Figure 1, the toy dataset, I think the violin plot above shows that it is the Blood Pressures from the *female subgroup* that increase linearly by age.  Blood Pressures of men have already increased earlier, in their twenties. Men generally have unhealthier values also for body weight, at earlier ages.

#### Comparing myself with men of my similar weight and BMI 

In the last years my weight was between 70 and 80 Kilos, and I was always slightly  to moderately overweight, so let's say my [Body Mass Index](https://en.wikipedia.org/wiki/File:BMI_chart.svg) varied between 25-30.


Let's build a new dataset `bloodpressure3` similar to the toy dataset from Figure 1.
...
```{r bpplot, fig.cap="Blood pressure as a function of age and body weight. NHANES data filtered to match my own age-, BMI-, and weight-range. Red dots are predicted values by the two-predictor model."}

...
The join reduced this to ~5000 athletes. Hence, many players didn't actually play, or the screenscraping process did not yield any information about the matches they 've participated in; or the parse-process of many downloaded files containing the Matches data failed.

### Histogram of year of births of players that actually played  

This is the "age distribution" of players in the dataset. I think the axes are pretty self-explanatory. The bumpy distribution is not, however.
...
Player_in_match %>%
        ...
        ggtitle(sprintf("Years of Births of %s athletes in the 'Players' database table", nrow(Player_in_match)),
                subtitle="All have entries in 'Matches' table (2008-2016), nominated for squad") +
        xlab("Year of Birth") +
...
oldest.2 <- map(oldest$player_name, 
                            kgapi_lookup_kv, apikey=Sys.getenv("GOO_KGR_KEY"))
...
The database design is pretty clean, and the fact that there are even some Primary-Foreign Key Constraints between the table inside the database make it stand out. However the mappings seem to be wrong, or I did a mistake in the analysis.

...
oldest.3 <- oldest.3 %>% 
   ...
   select(-result.name) %>% 
        mutate(Goalie = ifelse(str_detect( Description, "goal"), "Y", "")) %>% 
        mutate_if(is.numeric, function(x) ifelse(is.na(x), "", x)) %>% 
  select(Name, Description, Born, Goalie, N)
   

```

In the table below, column 'N' means number of matches in Database.
...
knitr::kable(oldest.3, table.attr = "id=\"oldestkg\"",
             ...
             align = c(rep("l", ncol(oldest.3)-1 ), "r"))
```

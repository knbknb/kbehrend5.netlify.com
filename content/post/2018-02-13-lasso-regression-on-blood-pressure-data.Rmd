...
```{r}
library(leaps)
regfit.full=regsubsets(BPSysAve~.,data=imputed)
summary(regfit.full)
```
...
```{r}
regfit.full=regsubsets(BPSysAve~.,data=imputed, nvmax=19)
reg.summary=summary(regfit.full)
names(reg.summary)
plot(reg.summary$cp,xlab="Number of Variables",ylab="Cp")
minimum <- which.min(reg.summary$cp)
points(minimum,reg.summary$cp[minimum],pch=20,col="red")
```
There is a plot method for the `regsubsets`  object
```{r}
plot(regfit.full,scale="Cp")
coef(regfit.full,minimum)
```
...
```{r}
regfit.fwd=regsubsets(BPSysAve~.,data=imputed,nvmax=19,method="forward")
summary(regfit.fwd)
...
dim(imputed)
set.seed(1)
# take 180 indexes of observations, 
train=sample(seq(263),180,replace=FALSE)
train
# use training data subset
regfit.fwd=regsubsets(BPSysAve~.,data=imputed[train,],nvmax=19,method="forward")
```
...
for(k in 1:10){
  ...
  best.fit=regsubsets(BPSysAve~.,data=imputed[folds!=k,],nvmax=19,method="forward")
  for(i in 1:19){
...
leg1str <- sprintf("log of best Lambda = %.3f  (%.3f)",log(cv.lasso$lambda.min), cv.lasso$lambda.min)
leg2str <- sprintf("log of best Lambda + 1 SE = %.3f (%.3f)",log(cv.lasso$lambda.1se), cv.lasso$lambda.1se)
plot(cv.lasso)
legend("topleft", legend=c(leg1str, leg2str), pch=1) 

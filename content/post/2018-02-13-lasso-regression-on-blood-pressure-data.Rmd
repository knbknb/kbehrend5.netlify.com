...
In the preceding blogpost I noticed that Systolic Blood Pressure is correlated with age and weight, but the relationship is complicated: the trend was noisy and the correlation was weak, and the  interaction term in the linear model seems to play an important role. 
...
These metadata are, for example, the unique distinct values of columns where it makes sense to report them (e.g. male/female for column "gender") 


#### Feature Engineering
Table 1 shows  the column names and (further below) some other important metadata of the NHANES dataset. 
...
Ther are no conclusions. The imputation procedure has created too much misleading information.
(TBC)
